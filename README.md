...
### 🎮 Game Server / Backend Programmer

서버 권위 멀티플레이 아키텍처와 백엔드 시스템을 직접 설계하고 구현합니다.<br/>
클라이언트(Unity)부터 게임 서버·백엔드·DB까지, 데이터가 흐르는 전 구간을 이해하고 만듭니다.
...
- **게임 서버·백엔드 개발자**로, 단순히 "무엇을 만들었다"가 아니라 **어떤 문제를 어떤 구조로 풀었는지**를 중요하게 생각합니다.
- **서버 권위(server-authoritative) 멀티플레이**, **이중 서버 아키텍처(gRPC + TCP 소켓)**, **메시지 기반 서버 간 연동(Redis Streams + Outbox)** 을 직접 설계하고 구현했습니다.
- **Clean Architecture** 기반의 유지보수 가능한 도메인 설계와, **단위 / 통합(Testcontainers) / E2E** 3계층 테스트로 회귀를 방지하는 개발을 지향합니다.
- 현재 **OnewaveGames**에서 게임 클라이언트 + 백엔드 개발(주니어 프로그래머)을 담당하고 있습니다.
...
**Server / Backend**
`C#` · `.NET 10` · `ASP.NET Core gRPC` · `TCP Socket` · `MemoryPack` · `EF Core` · `PostgreSQL` · `Redis (Streams · Cache)` · `Outbox 패턴`

**Client**
`Unity` · `C#` · `VContainer (DI)` · `UniTask` · `R3` · `MVI / MVVM` · `Addressables`
...
> **게임 서버 개발 역량을 증명하기 위한 실전 프로젝트.** 대부분의 게임 로직은 gRPC(HTTP/2)로, 극한 실시간 인게임은 TCP + MemoryPack으로 처리하며, 두 서버는 **직접 RPC 없이 Redis Streams(Outbox 패턴)** 로만 통신합니다.
...
`Unity 6` · `.NET 10` · `ASP.NET Core gRPC` · `TCP + MemoryPack` · `Redis Streams` · `PostgreSQL` · `Clean Architecture` · `Docker`
